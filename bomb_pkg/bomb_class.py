...
class Bomb_game:
    ...
    Exis_card = {'폭탄': b, '제거': d, '셔플': s, '투시': x, '강탈': t,
                 '스킵': p, '밑장빼기': u, '프라이팬x1': f1, '프라이팬x3': f3} # 카드이름

    Card_num_2 = [1, 2, 3, 3, 3, 2, 3, 3, 2] # 2인용 카드개수
    Card_num_3 = [1, 3, 3, 4, 4, 3, 4, 4, 2] # 3인용 카드개수
    Card_num_4 = [1, 3, 3, 5, 5, 3, 5, 4, 3] # 4인용 카드개수
    ...
    @staticmethod
    def setting_deck(pn): # pn은 플레이어 수; 2 3 4
        '''
        시작할 때 덱을 섞는 메소드
        단, 다음의 조건을 만족해야한다.
    ...
    def pri_xray(card):
        '''
        카드 메소드 xray() 에서 카드를 출력하는 보조 메소드이다.
        '''
        print(frame.split()[0])
        for l in Bomb_game.Exis_card[card].split() :
            print('⣿' + l + '⣿')
        print(frame.split()[1])
    ...
    def pri_board(self, card, main_board):
        ...
        '''
        for idx, line in enumerate(main_board.split()) :
            if (idx >= self.y) and (idx < self.y+16) :
                print(line[:self.x] + Bomb_game.Exis_card[card].split()[idx-self.y] + line[self.x+30:])
            else :
                print(line)
        print()
    ...
    def xray(self):
        ...
        try:
            print('윗장부터')
            for i in range(3) :
                Bomb_game.pri_xray(Bomb_game.Deck[i])
                print(Bomb_game.Deck[i])
                print()
    ...
    def frypan(self, oppo, n): # pl은 플레이어 리스트, n은 프라이팬 배수; 1 3
        '''
